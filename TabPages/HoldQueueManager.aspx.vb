...
Partial Class TabPages_HoldQueueManager
    Inherits MyBasePage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Session("_VT_OnHoldManagerRowSelected") Is Nothing Then
            ...
            Dim dt As DataTable = Session("_VT_OnHoldManagerRowSelected")
...
Errorhandler:
    ...
    Sub FillData()
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            If dtoutput IsNot Nothing AndAlso dtoutput.Rows.Count > 0 Then
                ...
                End If
                If dtoutput.Columns.Contains("MachineNum") = False Then
                    dtoutput.Columns.Add("MachineNum")
                End If
                If dtoutput.Columns.Contains("Kanban") = False Then
                    dtoutput.Columns.Add("Kanban")
                End If
                If dtoutput.Columns.Contains("ShowLocked") = False Then
                    dtoutput.Columns.Add("ShowLocked")
                End If
                ...
                For intCnt = 0 To dtoutput.Rows.Count - 1
                    ...
                    strformlock = objVTM.ReadFormLock(Session("_VT_DotNetConnString"), Me.CurrentSession.VT_OrderMatrixID, Me.CurrentSession.FormDataTable)
                    If strformlock <> "" Then 'it is locked
                        'show a lock icon in the grid
                        dtoutput.Rows(intCnt).Item("ShowLocked") = "YES"
                    Else
                        'show nothing in the grid
                        dtoutput.Rows(intCnt).Item("ShowLocked") = ""
                    End If

                    If IsDBNull(dtoutput.Rows(intCnt).Item("DateOnHold")) = False AndAlso dtoutput.Rows(intCnt).Item("DateOnHold") <> "" Then
                        dtoutput.Rows(intCnt).Item("DateOnHold") = DateDiff(DateInterval.Day, dtoutput.Rows(intCnt).Item("DateOnHold"), PortalFunctions.Now.Date)
                    Else
                        dtoutput.Rows(intCnt).Item("DateOnHold") = "0"
                    End If
                ...
                dtoutput = objG.SortDataTable("DateOnHold DESC, SO_ContiguousNum ASC, SalesItemNum ASC", dtoutput)
                ' dtoutput = objG.SortDataTable("DateOnHold DESC", dtoutput)
                Session("OnHoldDashboardGrid") = dtoutput
                objDataPreserve.BindDataToWDG(dtoutput, wdgRepGrid)
                Dim intNumDays As Integer

                'Apply Grid Formatting here
                For intLoop As Integer = 0 To dtoutput.Rows.Count - 1


                    If Trim(dtoutput.Rows(intLoop).Item("DateOnHold")) <> "" Then
                        intNumDays = dtoutput.Rows(intLoop).Item("DateOnHold")

                    Else
                        intNumDays = 0
                    End If

                    'if its on hold because of price then show it in bold
                    If InStr(dtoutput.Rows(intLoop).Item("ReasonOnHold"), "[System]  No Price") > 0 Then
                        If intNumDays >= 5 Then
                            'Set the formatting on the grid - Set the Background colour
                            wdgRepGrid.Rows(intLoop).CssClass = "Row_SalmonBold"
                        ElseIf intNumDays >= 3 Then
                            'Set the formatting on the grid - Set the Background colour
                            wdgRepGrid.Rows(intLoop).CssClass = "Row_YellowBold"
                        Else
                            wdgRepGrid.Rows(intLoop).CssClass = "Row_Bold"
                        End If
                    Else
                        If intNumDays >= 5 Then
                            'Set the formatting on the grid - Set the Background colour
                ...
                lblNumItems.Text = dtoutput.Rows.Count
    ...
    Protected Sub wdgRepGrid_ActiveCellChanged(sender As Object, e As Infragistics.Web.UI.GridControls.ActiveCellEventArgs) Handles wdgRepGrid.ActiveCellChanged
        ...
        If Not e Is Nothing Then
            ...
            If Not objC Is Nothing Then
                Session("_VT_OnHoldManagerRowSelected") = objC.SerialiseWebDataGridRow(wdgRepGrid, intActiveRowIndex)
    ...
    Protected Sub cmdSavePersonAssign_Click(sender As Object, e As ImageClickEventArgs) Handles cmdSavePersonAssign.Click
        If Session("_VT_OnHoldManagerRowSelected") IsNot Nothing Then
    ...
    Protected Sub btnViewOrder_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs)
        ...
        dtTemp = Session("_VT_OnHoldManagerRowSelected")
    ...
    Protected Sub wdgOrderItems_ActiveCellChanged(sender As Object, e As Infragistics.Web.UI.GridControls.ActiveCellEventArgs) Handles wdgOrderItems.ActiveCellChanged
        Session("_VT_OnHoldManagerRowSelected") = e.CurrentActiveCell.Row
        Session("_VT_OnHoldManagerRowSelectedIndex") = e.CurrentActiveCell.Row.Index
    End Sub
